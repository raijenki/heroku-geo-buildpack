test-heroku-16:
	...
	@docker run -v "$(shell pwd):/buildpack:ro" --rm -it -e "STACK=heroku-16" heroku/heroku:16-build bash -c '/buildpack/test/tests.sh'
	@echo ""

test-heroku-18:
	@echo "Running tests in docker (heroku-18)..."
	@docker run -v "$(shell pwd):/buildpack:ro" --rm -it -e "STACK=heroku-18" heroku/heroku:18-build bash -c '/buildpack/test/tests.sh'
	@echo ""

test-heroku-20:
	@echo "Running tests in docker (heroku-20)..."
	@docker run -v "$(shell pwd):/buildpack:ro" --rm -it -e "STACK=heroku-20" heroku/heroku:20-build bash -c '/buildpack/test/tests.sh'
	@echo ""
